...
std::string lower_expr_to(const Expr& e, const Module& module, std::vector<MirInsn>& out,
  ...
  switch (e.kind) {
    ...
    case Expr::Kind::Conditional: {
      // x if cond else y
      // Lower to: if cond then x else y
      const std::string dest = fresh_temp();
      const std::string else_label = fresh_label("else_");
      const std::string end_label = fresh_label("merge_");
      // Lower condition
      if (e.cond) {
        const std::string cond_dest = lower_expr_to(*e.cond, module, out, float_names, simd_names, i64_locals);
        push_branch_if_zero(out, cond_dest, else_label);
      }
      // Lower then branch
      if (e.operand) {
        const std::string then_dest = lower_expr_to(*e.operand, module, out, float_names, simd_names, i64_locals);
        MirInsn then_store;
        then_store.op = MirOp::StoreInt;
        then_store.ident = dest;
        then_store.lhs_ident = then_dest;
        out.push_back(std::move(then_store));
      }
      push_jump(out, end_label);
      // Else label
      push_label(out, else_label);
      // Lower else branch
      if (e.rhs) {
        const std::string else_dest = lower_expr_to(*e.rhs, module, out, float_names, simd_names, i64_locals);
        MirInsn else_store;
        else_store.op = MirOp::StoreInt;
        else_store.ident = dest;
        else_store.lhs_ident = else_dest;
        out.push_back(std::move(else_store));
      }
      // End label
      push_label(out, end_label);
      return dest;
    }
    case Expr::Kind::UnaryMinus: {
